...
def _camera_params(camera_config, role):
    return {
        "camera_name": _require_string(camera_config, "camera_name", f"cameras.{role}"),
        "camera_info_url": _require_string(camera_config, "camera_info_url", f"cameras.{role}"),
        "camera_serial_number": _require_string(camera_config, "serial_number", f"cameras.{role}"),
        "frame_id": _require_string(camera_config, "frame_id", f"cameras.{role}"),
    }
...
node_params = os.path.join(bringup_share, "config", "node_params.yaml")
...
recorder_node = Node(
    ...
    output="screen",
    emulate_tty=True,
    parameters=[{"config_path": os.path.join(bringup_share, "config", "topic_record_params.yaml")}],
)
