...
public:
    ...
    void drawResults(
        const cv::Mat & image, const cv::Point2f & center, double radius, double fitScore,
        bool found);
    // Debug msg;
    ...
    {
        Light() = default;
        explicit Light(cv::Point2f center, float radius) : center(center), radius(radius)
        {
            initializeLight(center, radius);
        };
        cv::Point2f center;
        float radius;
        double width;
    ...
    std::vector<Light> find_lights(
        const cv::Mat & color_image, const cv::Mat & binary_image);  // find lights in binary image
