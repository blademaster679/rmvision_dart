...
bool PnPSolver::solvePnP(
    const Detector::Light & light, std::vector<cv::Mat> & rvecs, std::vector<cv::Mat> & tvecs)
{
    // 准备图像点：中心及四个边界点
    std::vector<cv::Point2f> image_points{
        light.center, light.top, light.right, light.bottom, light.left};
    ...
    try {
        ...
        if (!ok || rvecs.empty() || tvecs.empty()) {
            RCLCPP_WARN(rclcpp::get_logger("pnp_solver"), "PnP 求解失败或未返回合法向量");
            return false;
        }
        return true;
    } catch (const cv::Exception & e) {
        RCLCPP_WARN(rclcpp::get_logger("pnp_solver"), "cv::solvePnPGeneric 异常: %s", e.what());
        return false;
    }
}
...
double PnPSolver::getDistance(const Detector::Light & light, cv::Mat & rvec, cv::Mat & tvec)
{
    ...
    if (cv::solvePnP(
            circle_points, image_points, camera_matrix, distortion_coefficients, rvec, tvec, false,
            cv::SOLVEPNP_ITERATIVE)) {
        std::vector<double> tvecVec;
        tvec.copyTo(tvecVec);
        double distance =
            std::sqrt(tvecVec[0] * tvecVec[0] + tvecVec[1] * tvecVec[1] + tvecVec[2] * tvecVec[2]);
